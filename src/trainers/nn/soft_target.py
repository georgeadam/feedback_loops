...
from src.utils.losses import SoftCE
from .regular import train_regular_nn

logger = logging.getLogger(__name__)


class SoftTargetNNTrainer:
    ...
    def update_fit(self, model, data_wrapper, rate_tracker, scaler, update_num, *args):
        ...
        # original version: (pred == 1) & (y_update == 1)
        soft_indices = (pred == 1) & (y_update == 1)
        ...
        loss_fn = SoftCE()
        model = train_regular_nn(model, self._optimizer, loss_fn, x_train, y_train, x_val, y_val,
