...
RESAMPLERS = {"smote": SMOTE, "edited_nn": EditedNearestNeighbours, "all_knn": AllKNN, "none": None}


class ImbalancedNNTrainer:
    def __init__(self, model_fn, seed, warm_start, update, optim_args, **kwargs):
        ...
        self._resampler = self._get_resampler(optim_args.resampler)
    ...
    def initial_fit(self, model, data_wrapper, scaler):
        ...
        train_imbalanced_nn(model, self._optimizer, F.cross_entropy, x_train, y_train, x_val, y_val,
                         self._epochs, self._early_stopping_iter, self._resampler, self._writer, "train_loss/0", self._write)
    ...
    def update_fit(self, model, data_wrapper, rate_tracker, scaler, update_num, *args):
        ...
        model = train_imbalanced_nn(model, self._optimizer, F.cross_entropy, x_train, y_train, x_val, y_val,
                                 self._epochs, self._early_stopping_iter, self._resampler, self._writer,
                                 "train_loss/{}".format(update_num), self._write)

        return model

    def _get_resampler(self, resampler):
        return RESAMPLERS[resampler]


def train_imbalanced_nn(model, optimizer, loss_fn, x_train, y_train, x_val, y_val, epochs, early_stopping_iter,
                        resampler, writer, writer_prefix, write=True):
    model.train()
    losses = []
    ...
    if resampler is not None:
        ...
        x_train = torch.from_numpy(x_resampled).float().to(model.device)
        y_train = torch.from_numpy(y_resampled).long().to(model.device)
    else:
