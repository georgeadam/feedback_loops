...
def update_model_conditional_trust(model, x_update, y_update, x_test, y_test, num_updates, physician_fpr,
                                   intermediate=False):
    np.random.seed(1)
    new_model = copy.deepcopy(model)
...
def update_model_feedback_with_training_cumulative(model, x_train, y_train, x_update, y_update, x_test, y_test,
                                                   num_updates, intermediate=False):
    np.random.seed(1)
    new_model = copy.deepcopy(model)
...
def update_model_feedback_linear_trend(model, x_update, y_update, x_test, y_test, num_updates, offset,
                                       intermediate=False):
    np.random.seed(1)
    new_model = copy.deepcopy(model)
