...
class DataWrapper:
    def __init__(self, include_train, ddp, ddr, tvp, agg_data):
    ...
    def accumulate_update_data(self):
        if self._agg_data:
            self._cumulative_x_update = np.concatenate((self._cumulative_x_update, self._x_update_current))
            self._cumulative_y_update = np.concatenate((self._cumulative_y_update, self._y_update_current))

    def store_current_update_batch(self, x, y):
        self._x_update_current = x
        self._y_update_current = y

    def get_current_update_batch(self):
        return self._x_update_current, self._y_update_current

    def _combine_all_data(self):
        if self._include_train:
            all_train_x = np.concatenate([self._x_train, self._cumulative_x_update, self._x_update_current])
            all_train_y = np.concatenate([self._y_train, self._cumulative_y_update, self._y_update_current])
        else:
            all_train_x = np.concatenate([self._cumulative_x_update, self._x_update_current])
            all_train_y = np.concatenate([self._cumulative_y_update, self._y_update_current])

        data = {"all_train_x": all_train_x, "all_train_y": all_train_y, "all_thresh_x": None, "all_thresh_y": None}

        return data

    def _all_data_helper_train(self):
        if self._include_train:
            ...
            y_threshold_set, y_threshold_reset = train_test_split(self._x_train, self._y_train, stratify=self._y_train,
                                                                  test_size=self._tvp)
            all_train_x = np.concatenate([x_threshold_set, self._cumulative_x_update, self._x_update_current])
            all_train_y = np.concatenate([y_threshold_set, self._cumulative_y_update, self._y_update_current])
    ...
    def _all_data_helper_update_current(self):
        neg_prop = np.sum(self._y_update_current == 0) / len(self._y_update_current)
        pos_prop = np.sum(self._y_update_current == 1) / len(self._y_update_current)

        if (int(neg_prop * self._tvp * len(self._y_update_current)) <= 1 or
                int(pos_prop * self._tvp * len(self._y_update_current)) <= 1):
            strat = None
        else:
            strat = self._y_update_current

        x_threshold_set, x_threshold_reset, \
        y_threshold_set, y_threshold_reset = train_test_split(self._x_update_current, self._y_update_current,
    ...
    def _all_data_helper_update_cumulative(self):
        temp_x = np.concatenate([self._cumulative_x_update, self._x_update_current])
        temp_y = np.concatenate([self._cumulative_y_update, self._y_update_current])
    ...
    def _all_data_helper_all(self):
        if self._include_train:
            temp_x = np.concatenate([self._x_train, self._cumulative_x_update, self._x_update_current])
            temp_y = np.concatenate([self._y_train, self._cumulative_y_update, self._y_update_current])
            ...
            neg_prop = (np.sum(self._y_update_current == 0) + np.sum(self._cumulative_y_update) == 0) / len(self._y_update_current)
            pos_prop = (np.sum(self._y_update_current == 1) + np.sum(self._cumulative_y_update == 1)) / len(self._y_update_current)

            if (int(neg_prop * self._tvp * len(self._y_update_current)) <= 1 or
                    int(pos_prop * self._tvp * len(self._y_update_current)) <= 1):
                strat = None
            else:
                strat = np.concatenate([self._cumulative_y_update, self._y_update_current])
    ...
    def get_all_data_for_model_fit(self):
        random_state = np.random.get_state()
        np.random.seed(1)
    ...
    def get_all_data_for_scaler_fit(self):
        if self._include_train:
            all_x = np.concatenate([self._x_train, self._cumulative_x_update, self._x_update_current])
        else:
            all_x = np.concatenate([self._cumulative_x_update, self._x_update_current])
...
class StaticDataWrapper(DataWrapper):
    def __init__(self, data, include_train, ddp, ddr, tvp, agg_data, num_updates):
        super().__init__(include_train, ddp, ddr, tvp, agg_data)
